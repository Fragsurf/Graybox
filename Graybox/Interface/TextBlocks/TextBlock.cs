	...
	public class TextBlock
	{
		...
		public void LoadMeasures( TextShaper textShaper = null )
		{
			...
			if ( GlyphSpan == null )
			{
				...
				MarginY = -fontMetrics.Ascent - FontHeight;
			}
		...
		public void GetLines( List<MeasuredSpan> result, float maximumwidth, float firstlinestart = 0, bool trimtrailingwhitespace = false )
		{
		...
		public int HitTest( float x, float y, SKRect rect, TextShaper textShaper = null )
		{
			LoadMeasures( textShaper );
			var lines = ListCache.Get();
			try
			{
				...
				float currentY = rect.Top + FontHeight + MarginY;
				...
				{
					var line = lines[i];
					float lineMidY = currentY + LineHeight / 2;
					float distance = Math.Abs( lineMidY - y );
				...
				if ( closestLineIndex != -1 )
				{
					...
					float lineX = rect.Left;
					if ( LineBreakMode == LineBreakMode.Center )
					{
						lineX += (rect.Width - line.width) / 2;
					}

					if ( x < lineX )
						x = lineX;
					else if ( x > lineX + line.width )
						x = lineX + line.width;

					return FindCharacterIndexAtX( line, x - lineX );
				}
		...
		private int FindCharacterIndexAtX( MeasuredSpan line, float x )
		{
			float accumulatedWidth = 0;
			int closestGlyphIndex = line.glyphstart;
			float closestGlyphDistance = float.MaxValue;
			float lastGlyphWidth = 0;

			for ( int index = line.glyphstart; index <= line.glyphend; index++ )
			{
				var glyphWidth = GlyphSpan.Measure( index, index ).width;
				lastGlyphWidth = glyphWidth;
				float glyphMidX = accumulatedWidth + glyphWidth / 2f;
				float distance = Math.Abs( glyphMidX - x );
				if ( distance < closestGlyphDistance )
				{
					closestGlyphDistance = distance;
					closestGlyphIndex = index;
				}
				accumulatedWidth += glyphWidth;
			}

			if ( x > line.width - lastGlyphWidth / 4 )
			{
				return line.glyphend + 1;
			}

			return closestGlyphIndex;
		}
